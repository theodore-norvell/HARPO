...
class TypeChecker(
  ...
  def typeCheck(exp: ExpNd): Option[Type] = {
    val ty: Option[Type] = exp match {
      ...
      case op @ AccessOp(pm) => {
        ...
        for(lsn <- op.p.locSet) {
          Some(typeCheckLocSet(lsn))
        }
        val newAmounts =
          for (permissionAmount <- pm.locExp) yield {
            val pTy = typeCheck(permissionAmount)
            val permissionAmount0 = valueConvert(permissionAmount);
            typeConvert(permissionAmount0, Some(real64))
          }
        op.p.setAmounts(newAmounts.toList)
        Some(real64) // since typeConvert set the permissions type to real64
      }
  ...
  def typeCheck(decl: DeclNd) {
    decl match {
      ...
      case decl @ ClaimNd(pm) => {
        typeCheckPermissionMap(pm)
      }
  ...
  def typeCheckPermissionMap(pmn: PermissionMapNd) {
    for (lsn <- pmn.locSet)
      typeCheck(lsn)
    val newAmounts =
      for (permissionAmount <- pmn.locExp) yield {
        val pTy = typeCheck(permissionAmount)
        val permissionAmount0 = valueConvert(permissionAmount);
        typeConvert(permissionAmount0, Some(real64))
      }
    pmn.setAmounts(newAmounts.toList)
  ...
  def typeCheck(mPerNd: PermissionNd) {
    mPerNd match {
      case mPerNd @ GivesPerNd(pmn) => {
        typeCheckPermissionMap(pmn)
      }
  ...
  def typeCheck(command: CommandNd) {
    command match {
        ...
        if (!methodType.isEmpty && argTypes.forall(_.isDefined)) {
          ...
          mty match {
            ...
            case _ => errorRecorder.reportFatal(
              s"Type error in call command. Expected a method, but found a ${mty}",
              method.coord)
          }
        }
      ...
      case CoForCmdNd(forDecl, repetitions, cl, body) => {
        for (cn <- cl) typeCheckPermissionMap(cn.pmn)
  ...
  private def typeCheckAssignments(lhss: Seq[ExpNd], rhss: Seq[ExpNd]): Seq[ExpNd] = {
  ...
  private def binaryOpType(exp: BinaryOpExpNd): Option[Type] = {
      ...
      case IndexOp =>
        ...
        if (exp.x.tipe == None || exp.y.tipe == None)
          ...
          if (!isIntegralType(yt)) {
            errorRecorder.reportFatal(
              s"Array index is of type ${xt}, but should be of integral type",
              exp.coord)
            None
          } else {
            xt match {
  ...
  private def unaryOpType(exp: UnaryOpExpNd): Option[Type] =
  ...
  private def isRealType(t: Type): Boolean = {
    t match {
      case `real16` | `real32` | `real64` => true;
      case _ => false;
    }
  }
